...
- replace contractions. abbreviations and acronyms
- process hashtags, usernames and urls


References : 
https://www.kaggle.com/code/gunesevitan/nlp-with-disaster-texts-eda-cleaning-and-bert
...
def remove_stopwords(text):
    union_stopwords = STOPWORDS.union(set(stopwords.words('english')))
    words = []
    for word in text.split(" "):
        word = word.lower()
        if ((word.isalpha()) and (word not in union_stopwords)):
            words.append(word) 
    return " ".join(words)
    

def clean_df(df:pd.DataFrame, field:str):
    print("Removing URLs ...")
    df[field] = df[field].map(remove_URL)
    print("Removing Emojis ...")
    df[field] = df[field].map(remove_emoji)
    print("Removing Punctuation ...")
    df[field] = df[field].map(remove_punct)
    print("Cleaning text ...")
    df[field] = df[field].map(clean)
    print("Lower case and remove stopwords ...")
    df[field] = df[field].map(remove_stopwords)
    return df
...
def main():
    ...
    for x in ["train", "test"]:
        ...
        df = pd.read_csv(f"./data/raw/{x}.csv")
        df = clean_df(df, "text")
        df.to_csv(f"./data/clean/{x}.csv", index=False)
